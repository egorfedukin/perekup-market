...
function renderContainers() {
  ...
  $("#container-grid").innerHTML = containers.map((box) => {
    ...
    return `<article class="container-card tier-${box.tier} ${box.viewerParticipated ? "player-lot" : ""} ${box.viewerParticipated && !leading ? "outbid-lot" : ""}" style="--box-color:${escapeHtml(box.color)}">${bidState ? `<div class="player-bid-status">${bidState}</div>` : ""}<div class="container-visual"><span>LOT ${box.id.slice(-4).toUpperCase()}</span><i></i><b>?</b></div><div class="container-info"><p class="eyebrow">${escapeHtml(box.label || "Закрытый")} контейнер</p><h3>${escapeHtml(box.name)}</h3><span>${escapeHtml(box.description || "Состав неизвестен до победы")}</span><small class="container-range">Диапазон: ${money(box.minValue)}–${money(box.maxValue)}</small><div class="container-bid-state"><strong>${money(current)}</strong><small>${box.highestBidderName ? `Лидирует ${escapeHtml(box.highestBidderName)}` : "Стартовая ставка"} · ставок ${box.bidCount}</small><time data-container-end="${box.endAt}">${auctionTime(box.endAt)}</time></div><form data-container-bid="${box.id}"><input name="amount" type="number" min="${minimum}" step="1000" value="${minimum}"><button class="primary-button" ${garageFull ? "disabled" : ""}>${garageFull ? "Нет места" : leading ? "Повысить ставку" : "Сделать ставку"}</button></form></div></article>`;
  }).join("");
}
