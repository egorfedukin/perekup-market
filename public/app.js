...
let bankRenderPending = false;
document.addEventListener("focusout", (event) => { if (bankRenderPending && event.target.closest?.("#bank-view")) setTimeout(() => { if (!$("#bank-view").contains(document.activeElement) && bankRenderPending) { bankRenderPending = false; render(); } }, 50); });
...
function renderBank() {
  ...
  $("#bank-products").innerHTML = productsHtml; document.querySelectorAll("[data-loan-product]").forEach((form) => updateLoanPreview(form));
  ...
  $("#bank-loans").innerHTML = loansHtml;
...
function render() {
  ...
  updateShell();
  if (activeView === "bank" && $("#bank-view").contains(document.activeElement) && document.activeElement.matches("input, select, textarea")) { bankRenderPending = true; return; }
...
document.addEventListener("submit", async (event) => {
  if (event.target.matches("[data-loan-product]")) {
    ...
    document.activeElement?.blur?.(); bankRenderPending = false;
