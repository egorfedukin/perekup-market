...
function buyerPrice({ value, fit, type, unknownCount, lied, relationship = 0, repaired = false, classic = false }) {
  ...
  return Math.round(value * Math.max(.65, base * fit + (repaired ? .035 : 0) - Math.min(.045, unknownCount * .009) - (lied ? .07 : 0) + Math.max(-10, Math.min(10, relationship)) * .003) / 1000) * 1000;
}
